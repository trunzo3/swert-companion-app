...
export default function Workshop() {
  ...
  // Listen for the unlock event from TopNav
  useEffect(() => {
    const handleUpdate = () => {
  ...
  // Default to first section or URL param later
  const firstUnlocked = sections.find(s => s.unlocked && s.tier === 1);
  const activeSectionId = firstUnlocked?.id || sections[0]?.id;
  
  const [selectedId, setSelectedId] = useState<string | null>(null);
  
  const currentId = selectedId || activeSectionId;
